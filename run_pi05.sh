...
for modality in ${MODALITIES}; do
    case "${modality}" in
        ...
        rgbd) modality_tag="${DEPTH_FUSION_MODE}_$(tr -d '[]' <<<"${DEPTH_TOKEN_GRID}" | tr ',' 'x')" ;;
    ...
    if [[ "${RUN_PHASE}" == eval || "${RUN_PHASE}" == all ]]; then
        echo "[INFO] eval ${modality}: tasks=${EVAL_TASKS}, checkpoint=${checkpoint}"
        if [[ "${DRY_RUN}" == false ]]; then
            VLA_CHECKPOINT="${checkpoint}" OUTPUT_DIR="${eval_dir}" TASKS="${EVAL_TASKS}" \
