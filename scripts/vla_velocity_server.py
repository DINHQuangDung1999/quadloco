#!/usr/bin/env python3
"""Serve a LeRobot waypoint or direct-velocity policy to Isaac Lab.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Serve VLA actions over a local TCP socket.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path(
    ...
    parser.add_argument(
        ...
        choices=("pi05", "smolvla"),
        default=None,
        help="Reject checkpoints of another policy type.",
    )
    return parser.parse_args()


def _queued_actions(policy: torch.nn.Module):
    """Return the policy's action queue across LeRobot policy implementations."""
    ...
    raise AttributeError(f"{type(policy).__name__} does not expose an action queue")
...
def main() -> None:
    ...
    if not (checkpoint / "config.json").is_file():
        raise FileNotFoundError(f"VLA config.json not found in checkpoint: {checkpoint}")

    device = torch.device(args.device)
    policy_cfg = _load_policy_config_compat(checkpoint)
    if args.expected_policy_type is not None and policy_cfg.type != args.expected_policy_type:
        raise ValueError(
            f"Expected a {args.expected_policy_type} checkpoint, received {policy_cfg.type}"
        )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        print(f"[INFO] {policy_cfg.type} {action_kind} server ready at {args.host}:{args.port}")
            ...
            with connection:
                ...
                try:
                    while True:
                        ...
                        action_queue = _queued_actions(policy)
                        ...
                        if action_np.shape != (action_dim,):
                            raise ValueError(
                                f"Expected VLA action shape ({action_dim},), received {action_np.shape}."
                            )
                        ...
                        if queue_was_empty:
                            queued_actions = list(action_queue)
                            if queued_actions:
