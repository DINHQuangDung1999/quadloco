...
class PI05Policy(PreTrainedPolicy):
    ...
    def _fix_pytorch_state_dict_keys(
        ...
        for key, value in state_dict.items():
            ...
            # The converted PI0.5 checkpoint stores PaliGemma's tied token
            # embedding under ``lm_head.weight``. Transformers expects the
            # same tensor at ``language_model.embed_tokens.weight``. Without
            # this alias, strict loading fails and the broad exception handler
            # above returns a policy whose pretrained weights were not fully
            # loaded.
            if (
